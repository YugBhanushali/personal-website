...
const Projects = () => {
  ...
  const scale = useTransform(scrollYProgress, [0.3, 0.5], [1, 1.6]);
  ...
  const cardGap = useTransform(scrollYProgress, [0, 0.6], ["80px", "30px"]);
  ...
  const xOdd = useTransform(scrollYProgress, [0, 0], ["-200px", "0px"]);

  const xEven = useTransform(scrollYProgress, [0, 0], ["200px", "-0px"]);
  ...
  return (
    <div
      ref={targetRef}
      id={"projects"}
      className="flex flex-col gap-6 items-center mt-[100px]"
    >
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
